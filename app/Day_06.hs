...
data Field2 t = F2 (M.Map (Int,Int) t) (Int, Int) | Out deriving(Eq)
...
indexWhere f (F2 m dim) = Prelude.filter pred [(a,b) | a <- [0..snd dim], b <- [0..snd dim]]
    where 
        pred x = 
...
look g@(Guard m p@(x, y) s l) = find m (addTup p $ dirToOffs l)
...
step g@(Guard f p s l) = 
    case (look g) of 
        ...
        Nothing  -> Guard Out (0,0) (p `S.insert` s) l 
...
day_06_a :: Guard -> Int
day_06_a g = length $ S.toList $ seen $ until (\x -> (patrol x) == Out) step g
